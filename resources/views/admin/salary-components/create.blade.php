<x-layouts.admin title="Tambah Komponen Gaji">
    <x-flash-message />
        ...
        <section class="page-hero">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-500">Buat jenis tunjangan atau potongan baru.</p>
                </div>
            </div>
        </section>

        <form action="{{ route('admin.salary-components.store') }}" method="POST" class="rounded-2xl border border-slate-200 bg-white p-6 md:p-8 shadow-sm">
            @csrf
            ...
            <div class="grid gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" 
                           ...
                           placeholder="Contoh: Tunjangan Wali Kelas, Potongan Koperasi..." required>
                ...
                <div>
                    ...
                    <select name="type" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                        ...
                        <option value="tunjangan" @selected(old('type') == 'tunjangan')>Tunjangan (Menambah Gaji)</option>
                        <option value="potongan" @selected(old('type') == 'potongan')>Potongan (Mengurangi Gaji)</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="default_amount" value="{{ old('default_amount', 0) }}" 
                           class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm focus:border-indigo-500 focus:ring-indigo-500" 
                           placeholder="0" required min="0">
                    <p class="mt-1.5 text-xs text-slate-500">Nominal ini akan otomatis terisi saat komponen diberikan ke karyawan, tapi bisa diubah (override) per karyawan.</p>
