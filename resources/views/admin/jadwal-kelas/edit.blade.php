<x-layouts.admin title="Edit Kelas — {{ $class->name }}">
    <x-flash-message />
    ...
    <div class="max-w-4xl mx-auto space-y-8" x-data="kelasForm()">
        ...
        <form action="{{ route('admin.rombel.update', $class) }}" method="POST"
            ...
            <div class="grid gap-8">
                    ...
                    <div>
                        ...
                        <select name="unit_id" x-model="selectedUnitId" @change="onUnitChange()"
                            ...
                            @foreach ($units as $unit)
                                <option value="{{ $unit->id }}" @selected(old('unit_id', $class->unit_id) == $unit->id)>{{ $unit->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="grid gap-6 md:grid-cols-3">
                    <div>
                        ...
                        <select name="level" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4 text-sm font-bold focus:border-teal-500 focus:ring-teal-500">
                            ...
                            <template x-for="level in availableLevels" :key="level">
                                <option :value="level" :selected="level == {{ old('level', $class->level ?? '') }}" x-text="'Tingkat ' + level"></option>
                            </template>
                        </select>
                    ...
                    <div>
                        ...
                        <select name="major" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4 text-sm font-bold focus:border-teal-500 focus:ring-teal-500">
                            ...
                            <option value="IPA" @selected(old('major', $class->major) === 'IPA')>IPA (MIPA)</option>
                            <option value="IPS" @selected(old('major', $class->major) === 'IPS')>IPS (IIS)</option>
                            <option value="Bahasa" @selected(old('major', $class->major) === 'Bahasa')>Bahasa</option>
                            <option value="Agama" @selected(old('major', $class->major) === 'Agama')>Keagamaan</option>
                        </select>
                ...
                <div>
                    ...
                    <select name="homeroom_teacher_id" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4 text-sm font-bold focus:border-teal-500 focus:ring-teal-500">
                        ...
                        @foreach ($teachers as $teacher)
                            <option value="{{ $teacher->id }}" @selected(old('homeroom_teacher_id', $class->homeroom_teacher_id) == $teacher->id)>{{ $teacher->employee->name }}</option>
                        @endforeach
                    </select>
                ...
                <label class="flex items-start gap-4 rounded-2xl border border-slate-200 bg-slate-50 px-5 py-4">
                    <input type="checkbox" name="allow_team_teaching" value="1" @checked(old('allow_team_teaching', $class->allow_team_teaching)) class="mt-1 rounded border-slate-300 text-teal-600 focus:ring-teal-500">
                    <span>
                        <span class="block text-sm font-black text-slate-800">Aktifkan Team Teaching</span>
                        <span class="mt-1 block text-xs font-medium text-slate-500">Saat aktif, slot yang sama boleh diisi lebih dari satu guru.</span>
                    </span>
                </label>
                ...
                <div class="pt-6 border-t border-slate-100 flex justify-end gap-4">
                    ...
                    <button type="submit" class="rounded-2xl bg-[#1a2744] px-8 py-4 text-sm font-black uppercase tracking-widest text-white shadow-lg hover:bg-slate-800 transition-all">Simpan Perubahan</button>
                </div>
            </div>
        </form>
    </div>
    ...
    function kelasForm() {
        ...
        const initUnitId = '{{ old('unit_id', $class->unit_id ?? auth()->user()->unit_id) }}';

        return {
            selectedUnitId: initUnitId,
            availableLevels: [],
            levelHint: '',
